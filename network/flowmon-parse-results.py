    ...
    for sim in sim_list:
        for flow in sim.flows:
            ...
            if flow.txBitrate is None:
                ...
                flow.txBitrate = np.nan
            else:
                print("\tTX bitrate: %.2f kbit/s" % (flow.txBitrate*1e-3,))
            if flow.rxBitrate is None:
                print("\tRX bitrate: None")
                flow.rxBitrate= np.nan
            else:
                print("\tRX bitrate: %.2f kbit/s" % (flow.rxBitrate*1e-3,))
            if flow.delayMean is None:
                print("\tMean Delay: None")
                flow.delayMean = np.nan
            else:
                print("\tMean Delay: %.2f ms" % (flow.delayMean*1e3,))
            if flow.packetLossRatio is None:
                print("\tPacket Loss Ratio: None")
                flow.packetLossRatio = np.nan
            else:
                print("\tPacket Loss Ratio: %.2f %%" % (flow.packetLossRatio*100))

            df = df.append({'origin': str(proto)+ str(t.sourceAddress)+'/'+str(t.sourcePort), \
                'arrival':  str(t.destinationAddress)+'/'+str(t.destinationPort),\
                 'TX bitrate kbit/s':flow.txBitrate*1e-3,\
                 'RX bitrate kbit/s':flow.rxBitrate*1e-3,
                 'Mean Delay ms':flow.delayMean*1e3
                 }, ignore_index=True)
    df.to_csv(argv[1][:-4]+'.txt', index=False)
